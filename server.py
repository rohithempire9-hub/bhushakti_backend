...
app=FastAPI(title='BHUSAKTHI Early Warning & Response API',version='4.1.0')
...
REGIONS=[{'id':a,'name':b,'district':c,'state':d,'lat':e,'lng':f,'susceptibility':g,'slope':h,'elevation':i,'road':j,'road_status':'OPEN','population_at_risk':k,'rainfall_mm':0,'soil_moisture_pct':0,'temperature_c':None,'humidity_pct':None,'wind_kmh':None,'data_status':'LIVE_WEATHER_OR_DEMO_FALLBACK','weather_source':'Open-Meteo','risk_source':'WEATHER_ADJUSTED_BASELINE'} for a,b,c,d,e,f,g,h,i,j,k in RAW]
...
def auth_user(authorization):
    ...
    try:
        ...
        if users is not None:
            user=users.find_one({'_id':data['id']})
            if user is None:
                try:
                    user=users.find_one({'_id':__import__('bson').ObjectId(data['id'])})
                except Exception: user=None
        else: user=next((u for u in memory_users if str(u['_id'])==data['id']),None)
        if not user: raise ValueError()
        return user
    except Exception: raise HTTPException(401,'Invalid or expired session')

def weather(lat,lng):
    q=urllib.parse.urlencode({'latitude':lat,'longitude':lng,'current':'temperature_2m,relative_humidity_2m,rain,precipitation,wind_speed_10m,soil_moisture_0_to_1cm','hourly':'rain,precipitation,soil_moisture_0_to_1cm','past_hours':24,'forecast_hours':24,'timezone':'auto'})
    with urllib.request.urlopen('https://api.open-meteo.com/v1/forecast?'+q,timeout=8) as r:return json.loads(r.read().decode())
def apply_weather(r):
    try:
        w=weather(r['lat'],r['lng']); c=w.get('current',{}); h=w.get('hourly',{}); rains=h.get('rain',[]); rain24=round(sum(float(v or 0) for v in rains[:24]),1); soil=float(c.get('soil_moisture_0_to_1cm') or 0)*100
...
@app.get('/health')
def health():return {'status':'ok','version':'4.1.0','weather':'Open-Meteo','database':'mongodb' if db is not None else 'memory','demo_mode':db is None}
