...
app = FastAPI(title='BHUSAKTHI Early Warning & Response API', version='4.0.0')
...
# Compact pilot metadata. Weather values are fallback/demo values; live weather replaces them when available.
RAW=[
('GSI-AR-01','Bhalukpong - Bomdila Corridor','West Kameng','Arunachal Pradesh',27.2645,92.4159,89,46,2150,'NH-13',4200),
('GSI-AR-02','Tawang - Bomdila Corridor','Tawang','Arunachal Pradesh',27.5867,91.8590,86,43,2650,'NH-13',2900),
('GSI-AR-03','Itanagar - Naharlagun Corridor','Papum Pare','Arunachal Pradesh',27.1020,93.6950,72,29,440,'NH-15',6100),
('GSI-AS-01','Haflong - Silchar Corridor','Dima Hasao','Assam',25.1640,93.0176,79,34,680,'NH-27',3500),
('GSI-ML-01','Cherrapunji - Mawsynram Escarpment','East Khasi Hills','Meghalaya',25.2986,91.5822,81,38,1430,'SH-5',3100),
('GSI-ML-02','Shillong - Jowai Corridor','East Khasi Hills / West Jaintia Hills','Meghalaya',25.4670,92.2030,74,33,1250,'NH-6',4700),
('GSI-MZ-01','Sairang - Aizawl Syncline','Aizawl','Mizoram',23.7271,92.7176,84,35,1132,'NH-54',3800),
('GSI-MZ-02','Lunglei Corridor','Lunglei','Mizoram',22.8877,92.7380,78,37,1220,'NH-302',2500),
('GSI-NL-01','Pfutsero - Kohima Ridge','Kohima','Nagaland',25.6751,94.1086,76,32,1444,'NH-29',2600),
('GSI-MN-01','Ukhrul - Imphal Corridor','Ukhrul / Imphal East','Manipur',24.7500,94.0100,77,36,1120,'NH-202',3300),
('GSI-SK-01','Gangtok - Rangpo Corridor','Gangtok / Pakyong','Sikkim',27.3389,88.6065,88,44,1650,'NH-10',5200)]
REGIONS=[]
for x in RAW:
    rid,name,district,state,lat,lng,slope_base_sus,slope,elev,road,pop=x
    REGIONS.append({'id':rid,'name':name,'district':district,'state':state,'lat':lat,'lng':lng,'susceptibility':slope_base_sus,'slope':slope,'elevation':elev,'road':road,'road_status':'OPEN','population_at_risk':pop,'rainfall_mm':0,'soil_moisture_pct':0,'temperature_c':None,'humidity_pct':None,'wind_kmh':None,'data_status':'LIVE_WEATHER_OR_DEMO_FALLBACK','weather_source':'Open-Meteo','risk_source':'WEATHER_ADJUSTED_BASELINE','villages':[]})
...
def pwd_hash(password,salt=None):
    salt=salt or secrets.token_hex(16); digest=hashlib.pbkdf2_hmac('sha256',password.encode(),salt.encode(),120000).hex(); return f'{salt}${digest}'
...
def auth_user(authorization):
    ...
    try:
        raw,sig=authorization[7:].split('.',1); data=json.loads(base64.urlsafe_b64decode(raw+'==')); secret=os.getenv('AUTH_SECRET','bhusakthi-demo-change-this-secret'); expected=hmac.new(secret.encode(),base64.urlsafe_b64decode(raw+'=='),'sha256').hexdigest()
        if not hmac.compare_digest(sig,expected) or data['exp']<time.time(): raise ValueError()
        users=collection('users'); user=users.find_one({'_id':__import__('bson').ObjectId(data['id'])}) if users is not None else next((u for u in memory_users if str(u['_id'])==data['id']),None)
        if not user: raise ValueError()
        return user
    except Exception: raise HTTPException(401,'Invalid or expired session')

def weather(lat,lng):
    params=urllib.parse.urlencode({'latitude':lat,'longitude':lng,'current':'temperature_2m,relative_humidity_2m,rain,precipitation,wind_speed_10m,soil_moisture_0_to_1cm','hourly':'rain,precipitation,soil_moisture_0_to_1cm','past_hours':24,'forecast_hours':24,'timezone':'auto'})
    url='https://api.open-meteo.com/v1/forecast?'+params
    with urllib.request.urlopen(url,timeout=8) as r:return json.loads(r.read().decode())

def apply_weather(r):
    try:
        w=weather(r['lat'],r['lng']); c=w.get('current',{}); h=w.get('hourly',{}); rains=h.get('rain',[]); rain24=round(sum(float(v or 0) for v in rains[:24]),1)
        soil=float(c.get('soil_moisture_0_to_1cm') or 0)*100
        r.update({'rainfall_mm':rain24,'rain_1h_mm':round(float(c.get('rain') or 0),1),'soil_moisture_pct':round(soil,1),'temperature_c':c.get('temperature_2m'),'humidity_pct':c.get('relative_humidity_2m'),'wind_kmh':c.get('wind_speed_10m'),'weather_source':'Open-Meteo LIVE','data_status':'LIVE_WEATHER','weather_updated_at':datetime.now(timezone.utc).isoformat()})
        return r
    except Exception:
        r['data_status']='DEMO_FALLBACK'; return r

def risk(r,rain_override=None,soil_override=None):
    rain=float(rain_override if rain_override is not None else r.get('rainfall_mm') or 0); soil=float(soil_override if soil_override is not None else r.get('soil_moisture_pct') or 0)
    structural=min(28,(r['susceptibility']*.18)+(r['slope']/50*100*.10))
    rain_factor=min(45,(rain/180)*45); soil_factor=min(27,(soil/100)*27)
    score=round(min(100,structural+rain_factor+soil_factor)); return score,('CRITICAL' if score>=75 else 'HIGH' if score>=55 else 'WATCH' if score>=35 else 'LOW')

def flood(r):
    rain=float(r.get('rainfall_mm') or 0); soil=float(r.get('soil_moisture_pct') or 0)
    s=round(min(100,rain/220*65+soil/100*35)); return s,'HIGH' if s>=75 else 'MODERATE' if s>=45 else 'LOW'
...
class Incident(BaseModel): sector_name:str; district:str=''; state:str=''; lat:float; lng:float; severity:str='HIGH'; status:str='ACTIVE'; type:str='Field observation'; notes:str=''; media:list[str]=[]
class RiskUpdate(BaseModel): region_id:str; rainfall_mm:float=Field(ge=0); soil_moisture_pct:float=Field(ge=0,le=100)

@app.get('/health')
def health(): return {'status':'ok','version':'4.0.0','weather':'Open-Meteo','database':'mongodb' if db is not None else 'memory','demo_mode':db is None}
...
def manual_risk(x:RiskUpdate):
    ...
    if not r: raise HTTPException(404,'Region not found')
    rr=dict(r); rr.update({'rainfall_mm':x.rainfall_mm,'soil_moisture_pct':x.soil_moisture_pct,'data_status':'MANUAL_SIMULATION'}); s,l=risk(rr); fs,fl=flood(rr)
    return {'region_id':x.region_id,'rainfall_mm':x.rainfall_mm,'soil_moisture_pct':x.soil_moisture_pct,'risk_score':s,'risk_level':l,'flood_score':fs,'flood_level':fl,'message':'Risk recalculated from rainfall + soil moisture'}
@app.get('/api/user/location-risk')
def location_risk(lat:float,lng:float,authorization:str=Header(None)):
    u=auth_user(authorization); best=min(REGIONS,key=lambda r:(r['lat']-lat)**2+(r['lng']-lng)**2); rr=region_view(best); return {'user':{k:u[k] for k in ['name','region_id','village']},'region':rr,'location':{'lat':lat,'lng':lng}}
...
def add_incident(x:Incident):
    d=x.model_dump(); d['id']='INC-'+secrets.token_hex(5).upper(); d['timestamp']=datetime.now(timezone.utc).isoformat(); c=collection('incidents'); c.insert_one(d) if c is not None else memory_incidents.insert(0,d); return d
...
def dispatch(alert_id:str):
    ...
    rc=collection('recipients'); rec=list(rc.find({}, {'_id':0})) if rc is not None else memory_recipients
    a.update({'status':'SIMULATED_SMS','recipient_count':len(rec),'delivered_count':len(rec),'provider':'SIMULATED_SMS','dispatched_at':datetime.now(timezone.utc).isoformat()})
    if c is not None:c.update_one({'id':alert_id},{'$set':a})
    return a

@app.get('/api/flood')
def flood_all(): return [region_view(r) for r in REGIONS]
